...
def filter_articles(data, keys_to_exclude):
    ...
    #         if not keys_to_exclude.contains(k):
    #             filtered_list.append(article)

    #return [article for article in data for k, v in article.items() if k not in keys_to_exclude]
    return [{k: v for k, v in article.items() if k not in keys_to_exclude} for article in data]
    #return [article if k not in keys_to_exclude for k,v in article.items() for article in data]
...
def count_words_by_year(data):
    ...
    for article in data:
        ...
        if subject_keyword in word_count_year.keys(): # Check if the subject_keyword variable is present in the keys of word_count_year.
...
def check_for_character_in_headline(article, character):
    ...
    for c in character:
        if c.lower() in article['headline']['main'].lower():
            return True
    return False
...
def star_wars_character_count(data, characters, character_count):
    ...
    #             if character in test_dict.keys():
    #                 test_dict[character] += 1
    #             else:
    #                 test_dict[character] = 1
    # return test_dict
    # return {k: sum(v.values()) / len(v.values()) for k, v in character_count.items()}
    # return [{k: v for k, v in article.items() if k not in keys_to_exclude} for article in data]
    return {character: character_count[character] + 1 
            for character in characters 
                for article in data 
                        if check_for_character_in_headline(article, character)}
...
def section_authors(data):
    ...
    # temp_list = []
    # for article in data:
    #     if not get_author_name(article) == None:
    #         temp_list.append({article['section_name']: get_author_name(article)})
    #return temp_list

    return [{article['section_name']:get_author_name(article)} for article in data if not get_author_name(article) == None]
